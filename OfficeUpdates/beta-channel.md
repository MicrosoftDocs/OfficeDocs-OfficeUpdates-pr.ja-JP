...
## <a name="version-2007-july-03"></a>バージョン 2007:03 年7月
...
- **クイックポールを使用して、Outlook で投票を作成します。** 投票を簡単に作成し、投票を収集して、メール内の結果を表示する[詳細情報](https://support.office.com/article/46893563-ab12-4bd0-aff7-26f5a488fea0)

- **新しい会議室のファインダー:** さまざまな機能を使用して、会議室を検索します。
...
- テーブルに関連付けられたクエリが編集されていないにもかかわらず、特定のバージョンの Excel で作成されたデータモデルのテーブルが ' Table Preview ' に表示されないという問題を修正しました。
- [名前の定義] ダイアログの [相対/絶対参照を無視する] を無効にすると、数式が機能しなくなるという問題を修正しました。
- 高度なデータフィルターをクリアするとテーブルの書式設定が失われるという問題を修正しました。
- 埋め込まれた PDF ドキュメントの完全なパスが、ファイル名だけではなくドキュメントキャプションに表示されるという問題を修正しました。
- Wolfram cloud コネクタを無効にしてから、Excel ブックを保存してから再度開くと、クラッシュする可能性があるという問題が修正されました。
- ソルバーアドインが有効になっている Excel を起動するとクラッシュが発生するという問題を修正しました。

### <a name="outlook"></a>Outlook

- 「To」行に130以上の受信者が存在する場合に Outlook がハングする問題を修正しました。また、テキストのレンダリングのパフォーマンスも向上しました。
- 「To Do Bar」では、2日以上にまたがるイベントと、それ以降のすべての日に同じ終了時刻が表示されるという問題を修正しました。
- Outlook のユーザーが、共有された予定表を使用して、数分間、メッセージ一覧の更新を停止するという問題を修正しました。

### <a name="powerpoint"></a>PowerPoint

- スライド上のテキスト領域に HTML を貼り付ける代わりに、スライドの上部に作成されたテキストボックスに貼り付けられるという問題を修正しました。
- 発表者ツールでスライドをすべて選択し、Alt + Tab を使用して発表者ツールを終了して、スライドショーに戻り、[終了] をクリックすると、処理不能な例外が発生するという問題が修正されました。

### <a name="project"></a>Project

- 特定の XML ファイルを開くと、Project がクラッシュすることがあるという問題を修正しました。
- ライブラリがモダンモードの場合は、SharePoint ドキュメントライブラリからプロジェクトファイルを開くことができないという問題を修正しました。
- URL が ' .com ' で終了している場合は、project Web App からプロジェクトデスクトップクライアントでプロジェクトを開くことができないという問題を修正しました。

### <a name="word"></a>Word

- 共同編集モードで問題を修正しました。マージの競合があり、ユーザーが変更を破棄するように選択している場合は、変更を保存または破棄するためのオプションが表示されなくなりました。
- マクロを含むファイルを新しい名前で保存しようとすると、ユーザーが入力した内容にかかわらず、そのファイルを保存して、ドキュメントを使用できないようにするという問題が修正さ WRO0004.docx れました。
...
- SharePoint/OneDrive にブックを保存するときに、カスタムリボンタブの CustomUI XML が削除されたという問題を修正しました。
- ファイルが読み取り専用のみを推奨している場合、ブックが読み取り専用であるという問題を修正しました。

### <a name="outlook"></a>Outlook

- 入力方式エディター (IME) ウィンドウが、さまざまな解像度の複数のモニターを使用しているときに、IME によって入力された基になるテキストと重なるという問題を修正しました。
- 以前に保存された予定を閉じるときに次のエラーが表示される問題を修正しました。 "別のユーザーまたは別のウィンドウで変更されたため、アイテムを保存できません。 アイテムの既定のフォルダーにコピーを作成しますか? "
- 日本のユーザーは、ミニカレンダーの日付が太字で表示されない問題を修正しました。
- 予定表のアラームで、会議が1週間未満であることを正確に表示できないという問題が修正されました。
...
- 固定期間タスクが100% 完了しているにもかかわらず、実績終了日が指定されていないという問題を修正しました。タスク% 完了は100% 未満と表示されます。

### <a name="word"></a>Word

- HTML ハイパーリンクの色が正しく表示されていない問題を修正しました。

### <a name="office-suite"></a>Office スイート

- Http または https ベースではない Url が最近使用した一覧に表示されていないという問題を修正しました。

[//]: # (BUGDETAILS コンテンツを削除しないでください。終了)

## <a name="version-2007-june-12"></a>バージョン 2007: 6 月12日
...
- リンクされた SQL Server テーブル内の Id 列を特定できない原因となった問題を修正しました。これにより、行が誤って削除されたとして報告されることがあります。

### <a name="excel"></a>Excel

- レーダーチャートの主要なグリッド線が正しく書式設定できない問題を修正しました。

### <a name="project"></a>Project

- プロジェクトのサマリータスクに変更があったとき、プロジェクトの開始/タスクフィールドのいずれかで ProjectBeforeTaskChange イベントが発生しなかったという問題を修正しました。
- 基準のリセットまたは更新によって、時間単位の予算コスト/作業リソースが変更され、基準値が正しくない予算値を表す可能性があるという問題を修正しました。

### <a name="word"></a>Word

- Office リボンの [書式のクリア] ボタンを使用して、[コメント] ウィンドウ内の書式をクリアする機能が機能しないという問題を修正しました。
- ルーラーが表示されない場合にテーブルのサイズを変更すると、バックグラウンドで実行されている他のアプリケーションのフラッシュが開始されるという問題を修正しました。
- 共同編集モードの場合、コメントの返信がコメントウィンドウに表示されないことがありますが、[変更履歴] ウィンドウに表示されることがあります。
- Word では、50個を超える頻繁に開かれたドキュメントがある場合、ドキュメントを保存して開くと、そのドキュメントに変更が加えられていなくても、改訂履歴が表示されるという問題を修正しました。
...
- マクロを含むファイルを新しい名前で保存しようとすると、ユーザーが入力した内容に関係なく、そのファイルを .docx 拡張子とファイル名 WRO0004.docx に保存すると、ドキュメントが使用できなくなりますという問題が修正されました。
