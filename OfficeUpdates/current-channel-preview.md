...
- **固定フォルダーに保存します。**[フォルダーを固定すると、Office ファイルの保存が容易になります。ユーザーが新しいファイルを保存するときに使用可能なフォルダーをより詳細に制御できるようにするためのフィードバックを受信します。 新しい機能の提供: [保存] ダイアログでフォルダーを固定します。 この新しい機能により、Word、Excel、および PowerPoint のファイルを簡単に保存できるようになります。 <br />[ブログの投稿](https://blog-insider.office.com/2020/05/18/pin-your-folders-makes-saving-office-files-easier/)の詳細情報をご覧ください
...
- **固定フォルダーに保存します。**[フォルダーを固定すると、Office ファイルを保存する easierWe には、ユーザーが新しいファイルを保存するときに使用できるフォルダーをより細かく制御できるようになります。 新しい機能の提供: [保存] ダイアログでフォルダーを固定します。 この新しい機能により、Word、Excel、および PowerPoint のファイルを簡単に保存できるようになります。 <br />[ブログの投稿](https://blog-insider.office.com/2020/05/18/pin-your-folders-makes-saving-office-files-easier/)の詳細情報をご覧ください
...
- SharePoint/OneDrive に保存するときに、カスタムリボンタブの CustomUI XML が削除される原因となった問題を修正しました。

### <a name="outlook"></a>Outlook

- クラウド設定が有効になっている場合に、Ctrl キーを押しながらクリックして動作を停止させた問題に対処しました。
...
## <a name="version-2006-june-11"></a>バージョン 2006: 6 月11日
...
- **ベクトル内のテキストを保持する:** これで、Excel、Word、および PowerPoint でこれらのオブジェクトを変換するときに、テキストを地図、グラフ、およびその他の SVG ベクトルで保持することができます。
...
- 使用可能なプリンターの一覧にプリンター名が複製される原因となった問題を修正しました。

- マージされた列を削除したユーザーのパフォーマンスが向上する原因となった問題を修正しました。

- ユーザーが指定した順序ではなくアルファベット順でアドインが読み込まれていたため、エラーメッセージ "このブックは現在、別のもので参照されていますが、閉じることができません" が表示されるという問題を修正しました。
...
- グラフシートにピボットテーブルを挿入しようとすると Excel がクラッシュすることがある問題に対処しました。
...
- 一部の Windows ビルドで Outlook がクラッシュする原因となった問題に対処しました。
...
- 修正された依存コードに回帰が発生しました。 これで、SharePoint Onprem 上で実行されている Visio services で画像が表示されるようになりました。

### <a name="word"></a>Word

- コメントウィンドウのタイムスタンプがシステムロケール時間に基づいていないという問題を修正しました。

- URL にクエリコンポーネントが含まれている場合に、カスタムドキュメント配信 (aspx) から Word 文書を開くときに問題が解決されました。
