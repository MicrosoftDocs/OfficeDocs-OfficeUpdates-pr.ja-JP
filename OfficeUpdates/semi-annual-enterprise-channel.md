...
- 既定では、アイテム保持ポリシーのラベルで、保持期間がかっこ内に表示されます。 これにより、管理者はポリシー名のみを表示するように指定するレジストリ キーを取得することができます。 HKCU\SOFTWARE\Microsoft\Office\16.0\Outlook\Preferences\HideRetentionPolicyDuration。 0 = 既定値 1 = アイテム保持ポリシー テキスト のPolicyName のみが表示されます。
